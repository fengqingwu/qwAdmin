<template>
    <div class="qw_form_box">
        ...
        <!-- <el-row> -->
            <!-- input 输入框或者 文本选择区域 -->
            <!-- <el-col :span="item.span?item.span:config.itemSpan" v-for="(item,i) in config.items" :key="i" :style="'height:' + (item.type == 'textarea'? '':'auto;')+ (item.colStyle? item.colStyle:'')"> -->
               <span class="span_form_item_box" v-for="(item,i) in config.items" :key="i" :style="'height:' + (item.type == 'textarea'? '':'auto;')+ (item.colStyle? item.colStyle:'')">
                ...
                <el-form-item 
                    ...
                    :prop="item.key" 
                    :label="item.name" 
                    :label-width="item.labelWidth? item.labelWidth :  parseInt(config.labelWidth) +'px'"
                    ...
                    <span slot="label">
                        <!-- -->
                        <el-tooltip v-if="(item.labelWidth)&&((parseInt(item.labelWidth)+10)/14 <= item.name.length)" class="item" effect="dark" :content="item.name" placement="top">
                           <span>{{item.name}}</span> 
                        </el-tooltip>    
                        <span v-else>{{item.name}}</span></span>                    
                ...
                <el-form-item v-if="item.type=='dateTimePicker'"
                    ...
                    <el-date-picker
                        ...
                        :placeholder="item.placeholder?item.placeholder: ((item.dateType=='date'||(item.dateType== undefined))? '选择日期':'选择日期时间')"
                ...
                <el-form-item v-if="item.type=='upload'"
                    ...
                    v-if="item.uploadType==undefined || (item.uploadType =='base')"
                    :multiple = "false"
                    ...
                    class="avatar-uploader"
                     v-if="item.uploadType =='photo'"
                    :action="item.action"
                    :show-file-list="false"
                    :on-success="item.handleAvatarSuccess"
                    :before-upload="item.beforeAvatarUpload">
                    <img v-if="config.formData[item.key]" :src="config.formData[item.key]" class="avatar">
                ...
                <!--ueditor -->
                <el-form-item v-if="item.type&& (item.type.toLocaleLowerCase()=='ueditor')"
                    :ref="'ueditor_'+ item.key"
                    :class="item.itemClass"
                    :label="item.name"
                    :prop="item.key"
                    :rules="item.rules? item.rules : [{required:false}]"                    
                    ...
                    <UE 
                    v-model="config.formData[item.key]" 
                    :config="item.editorConfig" 
                    :id="'ueditor_'+item.key"
                    @input="formItemInput(item)"
                    @focus="formItemFocus(item)"
                    @blur="formItemBlur(item)"
                    @change="formItemChange(item)"
                    :style="{width: item.controlWidth? item.controlWidth: (config.controlWidth? config.controlWidth: '')}"></UE>
                </el-form-item>
                <!-- 未完待续 -->
                <!-- 扩展具名插槽 -->
        ...
        <el-row > -->
            <p v-if="config.formBtns && config.formBtns.length>0" :style="'text-align:left;'+config.formBtnsStyle">
...
<script>
    import UE from '../ueditor/ueditor.vue';
    const config2={             
                labelWidth: 80,
                itemWidth:300,
                itemSpan:12,
...
export default {
    ...
    methods: {
        ...
        buildItemStyle(item,config){
           return 'width:' + (item.width? item.width : config.itemWidth?config.itemWidth +'px;': "auto;") + (item.formItemStyle? item.formItemStyle :'') + ';' + (config.itemStyle||"")
        },
        // 
