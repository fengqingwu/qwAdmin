               ...
               <span class="span_form_item_box"  v-for="(item,i) in config.items" :key="i" :style="'height:' + (item.type == 'textarea'? '':'auto;')+ (item.colStyle? item.colStyle:'')">
                ...
                <el-form-item 
                    ...
                    <el-input v-model.trim="config.formData[item.key]"
                            ...
                            <span class="subLabel" v-if="item.subLabel!=='' && (undefined!= item.subLabel)">{{item.subLabel}}</span>
                </el-form-item>
                <!-- 搜索下拉框 或下拉选择框 -->
                <el-form-item
                    :class="item.itemClass"
                ...
                <el-form-item v-if="item.type=='upload'"
                    ...
                    <span class="el-input el-input--mini" :style="'width:' + (item.inputWidth? item.inputWidth :'100px')+';'">
                        ...
                        <!-- <el-input style="width:100px;" v-model="config.formData[item.key]" readonly="readonly" :style="" size="mini"></el-input> -->
                ...
                <el-form-item  v-if="item.type&& (item.type.toLocaleLowerCase()=='qwrangeinput')"
                    :class="item.itemClass"
                    :label="item.name"
                    :prop="item.key"
                    :rules="item.rules? item.rules : [{required:false}]"
                    ...
                    <qwRangeInput 
                        ...
                        :min="item.min"
                        :max="item.max"
                        :splider="item.splider? item.splider : '/'"                      
                        :placeholder="item.placeholder?item.placeholder : '-'+ (item.splider? item.splider : '/') +'-'"
                        :size="item.size?item.size :'small'"
...
import qwRangeInput from "./qwRangeInput.vue"
export default {
    components:{
        qwRangeInput
    },
